...
"""
This code was modified from https://github.com/salesforce/factCC/blob/40a8a5684ec7131bfc92396d9b7f236f80095665/modeling/run.py
to allow for outputting predictions to a file.
"""
...
def test(args, model, tokenizer, prefix=""):
    """
    An edited version of `evaluate` that saves the predictions and does not write
    the results file.
    """
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_task_names = (args.task_name,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                probs = np.append(probs, torch.softmax(logits, dim=1).detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy(), axis=0)

        pred_labels = np.argmax(preds, axis=1)
        result = compute_metrics(args.task_name, pred_labels, out_label_ids)
        eval_loss = eval_loss / nb_eval_steps
        result["loss"] = eval_loss
        result["labels"] = pred_labels
        result["scores"] = probs[:, 1]
        results.update(result)
...
def main():
    ...
    if args.do_test:
        ...
        with open(args.output_file, "w") as out:
            for label, score in zip(results["labels_"], results["scores_"]):
                fcc_score.append(float(score))
                fcc_label = fcc_label + int(label)

            out.write(json.dumps({
                    "fcc_score": float(np.mean(fcc_score)),
                    "label_ratio": float(fcc_label/len(fcc_score)),
            }) + "\n")
